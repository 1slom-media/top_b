...
class CategoryController {
    ...
    public async Post(req: Request, res: Response) {
        try {
            ...
            const category = await AppDataSource.getRepository(CategoryEntity ).createQueryBuilder().insert().into(CategoryEntity ).values({ category_name_uz,category_name_ru }).returning("*").execute()
    ...
    public async Put(req: Request, res: Response) {
        try {
            ...
            const category = await AppDataSource.getRepository(CategoryEntity ).createQueryBuilder().update(CategoryEntity )
                .set({ category_name_uz,category_name_ru  })
                .where({ id })
                .returning("*")
                .execute()
